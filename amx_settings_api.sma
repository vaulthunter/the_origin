...
public native_load_setting_string_arr(plugin_id, num_params)
{
	...
	{
		log_error(AMX_ERR_NATIVE, "[Origin] Can't load settings: empty filename")
		return false;
	}
	...
	{
		log_error(AMX_ERR_NATIVE, "[Origin] Can't load settings: empty section/key")
		return false;
	}
	...
	{
		log_error(AMX_ERR_NATIVE, "[Origin] Array not initialized")
		return false;
	}
...
public native_save_setting_string_arr(plugin_id, num_params)
{
	...
	if (strlen(filename) < 1)
	{
		log_error(AMX_ERR_NATIVE, "[Origin] Can't save settings: empty filename")
		return false;
	}
	...
	if (strlen(setting_section) < 1 || strlen(setting_key) < 1)
	{
		log_error(AMX_ERR_NATIVE, "[Origin] Can't save settings: empty section/key")
		return false;
	}
