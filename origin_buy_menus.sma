...
new const MAXBPAMMO[] = { -1, 200, -1, 200, 1, 60, 1, 200, 200, 1, 200, 200, 200, 200, 200, 200, 200, 200,
			60, 200, 200, 60, 200, 200, 200, 2, 200, 200, 200, -1, 200 }
...
// Extra Items file
new const ORIGIN_SHOPITEMS_FILE[] = "origin_shoptems.ini"
...
public plugin_init()
{
	register_plugin("[Origin] Custom Buy Menus", ORIGIN_VERSION_STR_LONG, "ZP Dev Team")
	
	cvar_random_primary = register_cvar("origin_random_primary", "0")
	cvar_random_secondary = register_cvar("origin_random_secondary", "0")
	cvar_random_grenades = register_cvar("origin_random_grenades", "0")
	
	cvar_buy_custom_time = register_cvar("origin_buy_custom_time", "25")
	cvar_buy_custom_primary = register_cvar("origin_buy_custom_primary", "1")
	cvar_buy_custom_secondary = register_cvar("origin_buy_custom_secondary", "1")
	cvar_buy_custom_grenades = register_cvar("origin_buy_custom_grenades", "1")
	
	cvar_give_all_grenades = register_cvar("origin_give_all_grenades", "0")
	...
	register_clcmd("cl_rebuy", "clcmd_buy") 
	...
	g_Forwards[FW_ITEM_SELECT_PRE] = CreateMultiForward("origin_fw_items_select_pre", ET_CONTINUE, FP_CELL, FP_CELL, FP_CELL)
	g_Forwards[FW_ITEM_SELECT_POST] = CreateMultiForward("origin_fw_items_select_post", ET_IGNORE, FP_CELL, FP_CELL, FP_CELL)
}
...
public plugin_precache()
{
	...
	amx_load_setting_string_arr(ORIGIN_SETTINGS_FILE, "Buy Menu Weapons", "PRIMARY", g_primary_items)
	amx_load_setting_string_arr(ORIGIN_SETTINGS_FILE, "Buy Menu Weapons", "SECONDARY", g_secondary_items)
	amx_load_setting_string_arr(ORIGIN_SETTINGS_FILE, "Buy Menu Weapons", "GRENADES", g_grenades_items)
	...
	if (ArraySize(g_primary_items) == 0)
	{
		...
		amx_save_setting_string_arr(ORIGIN_SETTINGS_FILE, "Buy Menu Weapons", "PRIMARY", g_primary_items)
	}
	if (ArraySize(g_secondary_items) == 0)
	{
		...
		// Save to external file
		amx_save_setting_string_arr(ORIGIN_SETTINGS_FILE, "Buy Menu Weapons", "SECONDARY", g_secondary_items)
	}
	if (ArraySize(g_grenades_items) == 0)
	{
		...
		// Save to external file
		amx_save_setting_string_arr(ORIGIN_SETTINGS_FILE, "Buy Menu Weapons", "GRENADES", g_grenades_items)
	}
}

public plugin_natives()
{
	register_library("origin_buy_menus")
	register_native("origin_buy_menus_show", "native_buy_menus_show")
	register_native("origin_register_buy_item", "native_register_item")
...
public native_register_item(plugin_id, num_params)
{
	g_ItemCount = WPN_MAXIDS
	...
	if (strlen(name) < 1)
	{
		log_error(AMX_ERR_NATIVE, "[Origin] Can't register item with an empty name")
		return ORIGIN_INVALID_ITEM;
	}
	
	if ( type != ORIGIN_TYPE_PRIMARY && type != ORIGIN_TYPE_SECONDARY && type != ORIGIN_TYPE_NADE )
	{
		log_error(AMX_ERR_NATIVE, "[Origin] Can't register item with an invalid type")
	...
	{
		ArrayGetString(g_primary_items/*g_ItemRealName*/, index, item_name, charsmax(item_name))
		if (equali(name, item_name))
		{
			log_error(AMX_ERR_NATIVE, "[Origin] Item already registered (%s)", name)
			return ORIGIN_INVALID_ITEM;
		}
	}
	
	// Load settings from shop items file
	new real_name[32]
	copy(real_name, charsmax(real_name), name)
	ArrayPushString(g_ItemRealName, real_name)
	ArrayPushString(g_primary_items, real_name)
	
	// Name
	if (!amx_load_setting_string(ORIGIN_SHOPITEMS_FILE, real_name, "NAME", name, charsmax(name)))
		amx_save_setting_string(ORIGIN_SHOPITEMS_FILE, real_name, "NAME", name)
	ArrayPushString(g_ItemRealName, name)
	
	// Cost
	if (!amx_load_setting_int(ORIGIN_SHOPITEMS_FILE, real_name, "COST", cost))
		amx_save_setting_int(ORIGIN_SHOPITEMS_FILE, real_name, "COST", cost)
	ArrayPushCell(g_ItemCost, cost)
	
	// Type
	if (!amx_load_setting_int(ORIGIN_SHOPITEMS_FILE, real_name, "TYPE", type))
		amx_save_setting_int(ORIGIN_SHOPITEMS_FILE, real_name, "TYPE", type)
	ArrayPushCell(g_ItemType, type)
	flag_set(g_ShopItem, g_ItemCount)
...
public clcmd_buy(id)
{
	if (WPN_AUTO_ON)
	{
		origin_colored_print(id, "%L", id, "BUY_ENABLED")
		WPN_AUTO_ON = 0
	}
...
public human_weapons(id)
{
	...
	if (get_pcvar_num(cvar_buy_custom_primary))
	{
		...
		{
			if ( flag_get(g_ShopItem, WPN_AUTO_PRI) )
				buy_item(id, WPN_AUTO_PRI)
			else
...
show_menu_buy_primary(id)
{
	...
	if (menu_time <= 0)
	{
		origin_colored_print(id, "%L", id, "BUY_MENU_TIME_EXPIRED")
		return;
	}
	...
	{
		if ( flag_get(g_ShopItem, index) )
		{
			ArrayGetString(g_primary_items, index, weapon_name, charsmax(weapon_name))
			len += formatex(menu[len], charsmax(menu) - len, "\r%d.\w %s^n", index-WPN_STARTID+1, weapon_name)
		}
		else
		{
			ArrayGetString(g_primary_items, index, weapon_name, charsmax(weapon_name))
...
buy_item(id, itemid, ignorecost = 0)
{
	...
	if (g_ForwardResult >= ORIGIN_ITEM_NOT_AVAILABLE)
		return;
